...
export function installElementContextMenu(deps: ContextMenuDependencies): void {
	...
	chrome.contextMenus.onClicked.addListener((info, tab) => {
		...
		void shareContextMenuElement(info, tab, deps).catch((error) => {
			deps.recordDebug({ level: "warn", event: "context-menu-share-failed", message: error instanceof Error ? error.message : String(error), data: { tabId: tab?.id } });
		});
	});
}
...
async function shareContextMenuElement(info: ContextMenuInfo, tab: chrome.tabs.Tab | undefined, deps: ContextMenuDependencies): Promise<void> {
	...
	const response = await chrome.tabs.sendMessage(tab.id, {
		...
		options: { mode: "single", includeHtml: false, includeText: true, maxHtmlChars: 0, source: "context-menu" },
	}, typeof info.frameId === "number" ? { frameId: info.frameId } : undefined);
	const selectedAt = Date.now();
	deps.recordDebug({ level: "info", event: "context-menu-selection-shared", data: { tabId: tab.id, frameId: info.frameId, status: responseStatus(response), elementCount: responseElementCount(response) } });
