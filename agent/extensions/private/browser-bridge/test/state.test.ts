...
test("status summary includes latest shared drawing details", () => {
	...
	runtime.state.sharedDrawings.push({
		...
		previewImage: { path: "/tmp/drawing.png", mediaType: "image/png" },
...
test("state payload is a defensive copy", () => {
	...
	runtime.state.sharedDrawings.push({ drawingId: "drawing-1", clientId: "client-a", status: "drawn", sharedAt: 1237, pointCount: 1, strokes: [{ points: [{ x: 1, y: 2 }] }], gesture: { type: "arrow", start: { x: 1, y: 2 }, end: { x: 3, y: 4 }, fromElement: { selectorCandidates: [".from"] } }, previewImage: { path: "/tmp/drawing.png", crop: { x: 1, y: 2, width: 3, height: 4 } }, nearbyElements: [{ selectorCandidates: [".price"], boundingBox: { x: 1, y: 2, width: 3, height: 4 } }] });
	const snapshot = browserBridgeStatePayload(runtime.state);
