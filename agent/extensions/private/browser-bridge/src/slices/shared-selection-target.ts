...
export function resolveSharedElementTarget(runtime: BrowserBridgeRuntime, target: BrowserTarget, input: SharedStyleElementInput | undefined, options: { fallbackSelectionOffset: number; role: string }): ResolvedBrowserElementTarget {
	if (input?.elementId) {
		const resolved = resolveElementDescriptorByElementId(runtime, target, input.elementId);
		return resolved ? { elementId: input.elementId, selectionId: resolved.selection.selectionId, selectionIndex: resolved.index, expected: resolved.element, ...frameTargetFromSelection(resolved.selection) } : { elementId: input.elementId };
	}
	...
	const base = { selectionId: selection.selectionId, selectionIndex: index, expected: element, ...frameTargetFromSelection(selection) };
...
export function resolveElementDescriptorByElementId(runtime: BrowserBridgeRuntime, target: SharedSelectionLookupTarget, elementId: string): { selection: BrowserSharedSelectionSummary; index: number; element: BrowserElementDescriptorSummary } | undefined {
	...
	for (const selection of [...matching].reverse()) {
		const index = selection.elements.findIndex((element) => element.elementId === elementId);
		if (index >= 0) return { selection, index, element: selection.elements[index]! };
	}
	return undefined;
}
...
function frameTargetFromSelection(selection: BrowserSharedSelectionSummary): { frameId?: number } {
	const frameId = selection.context?.frameId;
	return typeof frameId === "number" && Number.isSafeInteger(frameId) && frameId >= 0 ? { frameId } : {};
}
