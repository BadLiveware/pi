...
Use `stardock_brief_worker({ action: "run", role: "explorer" | "test_runner" })` when the parent/governor explicitly wants to run the active or selected brief through `pi-subagents` without a payload-copying round trip. This is not automatic on brief creation. It builds the brief-scoped advisory payload, starts one `pi-subagents` worker through the extension bridge, defaults to fresh context and file-only output under `.stardock/runs/<loop>/workers/`, then records a compact WorkerReport unless `recordResult: false` is passed. Use it for advisory mapping or bounded validation only; do not use it for implementer/editing workers, automatic patch application, hidden fanout, or replacing parent/governor judgment.

If outside-help/governor requests appear, inspect them with `stardock_outside_requests`, fetch ready-to-copy work with `stardock_outside_payload`, satisfy them manually or with a parent/orchestrator workflow, then record answers with `stardock_outside_answer`. Use `stardock_govern` for an immediate manual governor review request and payload without automatically spawning subagents. Stardock keeps governor requests to one per iteration, so a manual governor request/decision suppresses the automatic cadence request for that same iteration. For active briefs that need only advisory explorer/test-runner help, prefer `stardock_brief_worker({ action: "run" })` over `stardock_advisory_adapter` followed by a separate `subagent` call.
