...
Use `stardock_final_report` before claiming substantial work complete when criteria/evidence/gaps need a durable summary. `record` stores compact manual reports with status (`draft`, `passed`, `failed`, or `partial`), summary, covered `criterionIds`, referenced `artifactIds`, validation records, unresolved gaps, and compatibility/security/performance notes; pass `reports: [...]` for batch writes or single-report fields for one-item batches. `list` inspects reports without reading `.stardock` files. Reports are optional evidence summaries in this slice: they do not run validators, call models, spawn auditors, or block completion automatically.
