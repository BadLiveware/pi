...
4. Work one bounded iteration. For non-trivial implementation or governance-request help, prefer `stardock_worker` so the governor keeps context, Stardock owns the role contract, and the parent can choose a worker model that fits task complexity.
