...
4. Work one bounded iteration. For non-trivial active-brief implementation, default to `stardock_worker({ action: "run", role: "implementer", briefId })` before parent `edit`/`write` so the governor keeps context, Stardock owns mutability and evidence, and the parent can choose a worker model that fits task complexity. Use explorer/test_runner/reviewer/auditor workers for mapping, validation, or review only; those roles do not satisfy implementation delegation.
