...
| `stardock_brief_worker` | Explicitly run an active or selected brief through the `pi-subagents` bridge for advisory explorer/test-runner work, save output under `.stardock/runs/<loop>/workers/` by default, and record a compact WorkerReport unless disabled. |
...
Use `stardock_brief_worker({ action: "run", role: "explorer" | "test_runner" })` when the parent/governor wants to explicitly start that brief-scoped advisory worker without a payload-copying round trip. This tool builds the same brief payload, sends one request through the `pi-subagents` bridge, defaults to `context: "fresh"`, saves returned output to `.stardock/runs/<loop>/workers/` with `outputMode: "file-only"`, and records the compact result as a WorkerReport unless `recordResult: false` is passed. It is not automatic on brief creation and is intentionally limited to advisory explorer/test-runner roles; implementer/editing workers, background fanout, provider-specific state contracts, and automatic patch application remain out of scope.
...
The extension does not spawn subagents automatically. A parent/orchestrator agent can inspect requests, fetch a ready-to-copy task with `stardock_outside_payload` or `/stardock outside payload`, run whatever research or review is appropriate, then record the result with `stardock_outside_answer` or `/stardock outside answer`. For active briefs that only need advisory codebase mapping or bounded validation, the parent/governor may instead call `stardock_brief_worker({ action: "run" })` to start one explicit `pi-subagents` worker and record its WorkerReport in a single tool call.
