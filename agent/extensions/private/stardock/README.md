...
It solves those problems by turning work into governed loops with compact state. Checklist mode scopes finite work through bounded iteration briefs, criteria, and evidence. Recursive mode runs one evaluated hypothesis per iteration. Stardock also extends the useful lifetime of outside-agent context: implementation delegation, worker reports, handoff records, and compact evidence packets preserve the minimum information another worker, auditor, governor, or future resumed agent needs to understand the work without inheriting the whole chat. The governor should prefer brief-scoped workers for non-trivial implementation so parent context stays compact and each worker can use a model matched to the brief's complexity. Ledgers, attempt reports, final reports, auditor/breakout records, outside requests, and policy checks keep validation, decisions, and oversight inspectable without making raw chat history or `.stardock` files the only durable project knowledge.
...
| `stardock_worker` | Run a Stardock-owned worker role through the `pi-subagents` bridge for brief-scoped or outside-request-scoped work, optionally pass a subagent model override, save output under `.stardock/runs/<loop>/workers/` by default, classify the result, record a WorkerRun/WorkerReport unless disabled, and require review before another implementer run. |
...
Use `stardock_advisory_adapter({ action: "payload", role: "explorer" | "test_runner", model })` when the parent wants a ready-to-run invocation for the active or selected brief without starting it. The adapter output is convenience text plus structured invocation details only: Stardock does not execute providers, spawn agents, persist provider-specific state, or apply edits from this payload surface. The invocation includes Stardock-owned role prompts; transport agent names are implementation details. If overriding `model`, first use `list_pi_models` and choose an enabled/supported model whose capability and cost fit the brief. Prefer `stardock_worker` when you want Stardock to execute and record the worker in one tool call.

Use `stardock_worker({ action: "run", role, briefId?, requestId?, model? })` when the parent/governor wants to explicitly start a Stardock-owned worker role through the bridge. Brief-scoped roles are `explorer`, `test_runner`, `implementer`, and `reviewer`; outside-request or loop-scoped governance roles are `governor`, `auditor`, `researcher`, and `reviewer`. This is the preferred path for non-trivial active-brief implementation and pending governance requests because the prompt, mutability, result classification, WorkerRun, and WorkerReport are owned by Stardock rather than a raw provider session. Before passing a non-default `model`, use `list_pi_models` and choose an enabled/supported model whose capability and cost fit the complexity. Do direct parent edits only for trivial/surgical changes, an unavailable or unsafe worker bridge, or an explicit gate/user decision. Read-only roles explicitly forbid edits; implementer is a serial mutable worker that requires a clean git workspace by default, records changed-file hints from git status, and must be reviewed with `stardock_worker({ action: "review", runId, reviewStatus: "accepted" | "dismissed" })` before another implementer or completion. The tool defaults to `context: "fresh"`, saves returned output to `.stardock/runs/<loop>/workers/` with `outputMode: "file-only"`, records a compact WorkerReport unless `recordResult: false` is passed, and marks pending outside requests answered when a request-scoped worker succeeds. It is not automatic on brief creation; background fanout, workspace isolation, provider-specific state contracts, and automatic patch application remain out of scope. `stardock_brief_worker` remains a compatibility wrapper for brief-scoped `explorer`, `test_runner`, and `implementer` runs.
...
The extension does not spawn subagents automatically. A parent/orchestrator agent can inspect requests, fetch a ready-to-copy task with `stardock_outside_payload` or `/stardock outside payload`, run whatever research or review is appropriate, then record the result with `stardock_outside_answer` or `/stardock outside answer`. Automatic auditor triggers create only these durable requests; they do not call a model or reviewer. For active briefs or outside requests that need codebase mapping, bounded validation, governance review, research, or one serial scoped implementation, the parent/governor should prefer `stardock_worker({ action: "run" })` to start one explicit bridge worker and record its WorkerReport in a single tool call, using a `model` override only after checking enabled/supported models with `list_pi_models`.
