...
The extension does not spawn subagents automatically. A parent/orchestrator agent can inspect requests, fetch a ready-to-copy task with `stardock_outside_payload` or `/stardock outside payload`, run whatever research or review is appropriate, then record the result with `stardock_outside_answer` or `/stardock outside answer`. For active briefs that need codebase mapping, bounded validation, or one serial scoped implementation, the parent/governor may instead call `stardock_brief_worker({ action: "run" })` to start one explicit `pi-subagents` worker and record its WorkerReport in a single tool call.
