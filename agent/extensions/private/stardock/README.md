...
The extension does not spawn subagents automatically. A parent/orchestrator agent can inspect requests, fetch a ready-to-copy task with `stardock_outside_payload` or `/stardock outside payload`, run whatever research or review is appropriate, then record the result with `stardock_outside_answer` or `/stardock outside answer`. Automatic auditor triggers create only these durable requests; they do not call a model or reviewer. For active briefs or outside requests that need codebase mapping, bounded validation, governance review, research, or one serial scoped implementation, the parent/governor should prefer `stardock_worker({ action: "run" })` to start one explicit bridge worker and record its WorkerReport in a single tool call, using `model` or `thinking` overrides only after checking enabled/supported models and `thinkingLevels` with `list_pi_models`.
