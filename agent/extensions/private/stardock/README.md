...
| `stardock_brief_worker` | Explicitly run an active or selected brief through the `pi-subagents` bridge for explorer/test-runner/serial implementer work, save output under `.stardock/runs/<loop>/workers/` by default, record a compact WorkerReport unless disabled, and require review before another implementer run. |
...
Use `stardock_advisory_adapter({ action: "payload", role: "explorer" | "test_runner" })` when the parent wants a ready-to-run `pi-subagents` invocation for the active or selected brief without starting it. The adapter output is convenience text plus structured invocation details only: Stardock does not execute providers, spawn agents, persist provider-specific state, or apply edits from this payload surface. Explorer payloads default to the `scout` subagent and ask for a read-next map; test-runner payloads default to `delegate` and ask for bounded validation evidence. The parent remains responsible for invoking the worker, inspecting the result, recording compact findings, and running `stardock_policy({ action: "parentReview" })` when risk warrants.

Use `stardock_brief_worker({ action: "run", role: "explorer" | "test_runner" | "implementer" })` when the parent/governor wants to explicitly start a brief-scoped worker without a payload-copying round trip. Explorer and test-runner remain advisory. Implementer is a serial mutable worker: Stardock requires a clean git workspace by default, refuses to start another implementer while one is running or needs review, records a durable WorkerRun plus WorkerReport, captures changed-file hints from git status, and requires `stardock_brief_worker({ action: "review", runId, reviewStatus: "accepted" | "dismissed" })` before another implementer or completion. The tool sends one request through the `pi-subagents` bridge, defaults to `context: "fresh"`, saves returned output to `.stardock/runs/<loop>/workers/` with `outputMode: "file-only"`, and records the compact result as a WorkerReport unless `recordResult: false` is passed. It is not automatic on brief creation; background fanout, workspace isolation, provider-specific state contracts, and automatic patch application remain out of scope.
...
The extension does not spawn subagents automatically. A parent/orchestrator agent can inspect requests, fetch a ready-to-copy task with `stardock_outside_payload` or `/stardock outside payload`, run whatever research or review is appropriate, then record the result with `stardock_outside_answer` or `/stardock outside answer`. Automatic auditor triggers create only these durable requests; they do not call a model or reviewer. For active briefs that need codebase mapping, bounded validation, or one serial scoped implementation, the parent/governor may instead call `stardock_brief_worker({ action: "run" })` to start one explicit `pi-subagents` worker and record its WorkerReport in a single tool call.
