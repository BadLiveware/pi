...
test("impact map skips whole-repo parsing for unsupported changed-file languages", async () => {
	...
	fs.writeFileSync(path.join(repo, "src", "query.java"), "class Query { void applyFunctionOverRange() {} }\n");
	fs.writeFileSync(path.join(repo, "src", "quantile.java"), "class AggregateFunctionTimeseriesQuantile {}\n");
	fs.writeFileSync(path.join(repo, "supported.ts"), "export function shouldNotNeedParsing() { return true }\n");

	const impact = parseToolResult(await loadTools().get("code_intel_impact_map")!.execute("test", { changedFiles: ["src/query.java", "src/quantile.java"], maxResults: 10 }, undefined, undefined, mockContext(repo)));
