...
test("registered tool surface includes targeted context and mutation tools", async () => {
	const tools = loadTools();
	assert.deepEqual([...tools.keys()].sort(), ["code_intel_file_outline", "code_intel_impact_map", "code_intel_insert_relative", "code_intel_local_map", "code_intel_post_edit_map", "code_intel_read_symbol", "code_intel_replace_symbol", "code_intel_repo_overview", "code_intel_repo_route", "code_intel_state", "code_intel_syntax_search", "code_intel_test_map"]);
});
...
test("state reports Tree-sitter and rg without legacy artifact policy", async () => {
	...
	assert.equal(state.semanticProviders.clangd.details.capabilities.diagnostics, "implemented");
	assert.equal(state.semanticProviders["csharp-ls"].details.capabilities.references, "implemented");
	assert.equal(state.semanticProviders["csharp-ls"].details.capabilities.diagnostics, "implemented");
