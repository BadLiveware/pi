...
export function registerOrientationTools(pi: ExtensionAPI): void {
	...
	pi.registerTool({
		...
		parameters: Type.Object({
			...
			confirmReferences: Type.Optional(Type.Union([Type.Literal("gopls"), Type.Literal("typescript"), Type.Literal("clangd"), Type.Literal("rust-analyzer")], { description: "Optional exact-reference confirmation for matching source-code tests when applicable." })),
