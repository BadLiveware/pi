...
export function registerTargetedContextTools(pi: ExtensionAPI): void {
	...
	pi.registerTool({
		...
		parameters: Type.Object({
			...
			includeDiagnostics: Type.Optional(Type.Boolean({ description: "Collect current touched-file diagnostics from applicable providers such as TypeScript, gopls, Rust Analyzer, ShellCheck, zsh -n, or markdownlint-cli2, and use supplied diagnostics to prioritize follow-up locations. Default false unless diagnostics are supplied." })),
