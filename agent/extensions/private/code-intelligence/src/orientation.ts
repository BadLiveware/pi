...
const GENERATED_DIRS = new Set(["generated", "gen", "autogenerated"]);
...
const TEST_FILE_PATTERN = /(^|\/).*(\.test|\.spec)\.[cm]?[tj]sx?$|(^|\/).*_test\.go$|(^|\/)test_.*\.py$/i;
...
function tokensForPath(file: string): string[] {
	...
	return [...new Set(stem.split(/[^A-Za-z0-9]+|(?=[A-Z])/).map((part) => part.toLowerCase()).filter((part) => part.length >= 3))];
}
