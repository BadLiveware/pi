...
export const WATCHDOG_NUDGE_PROMPT = [
	...
	"Check the previous task or loop state. If all requested work is complete, stop: for Ralph respond with `<promise>COMPLETE</promise>`; otherwise briefly say no further action is needed. If unfinished in-scope work remains, continue from the next concrete step.",
].join("\n\n");
...
export default function compactionContinue(pi: ExtensionAPI): void {
	...
	function scheduleRecovery(compactionId: string, ctx: ExtensionContext, seedAnalysis?: CompactionRecoveryAnalysis): void {
		...
		pendingTimer = setTimeout(() => {
			...
			pi.appendEntry("compaction-continue", {
				compactionId,
				kind: recovery.kind,
				loop: activeLoop?.name ?? recovery.ralph?.prompt?.loop,
				iteration: activeLoop?.iteration || recovery.ralph?.prompt?.iteration,
			...
			pi.sendUserMessage(WATCHDOG_NUDGE_PROMPT);
		}, RECOVERY_DELAY_MS);
	}
	...
	function scheduleRalphIdleRecovery(ctx: ExtensionContext, prompt: RalphPromptInfo, reason: string): void {
		...
		pendingRalphIdleTimer = setTimeout(() => {
			...
			pi.appendEntry("compaction-continue", {
				kind: "ralph-stall",
				loop: activeLoop.name ?? prompt.loop,
				iteration: activeLoop.iteration || prompt.iteration,
				reason,
				promptKey: scheduledPromptKey,
				timestamp: new Date().toISOString(),
			});

			ctx.ui.notify("Active Ralph loop went idle after saying it would continue; sending watchdog nudge.", "info");
			pi.sendUserMessage(WATCHDOG_NUDGE_PROMPT);
		}, RALPH_IDLE_DELAY_MS);
	}
