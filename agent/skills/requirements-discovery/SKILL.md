...
3. Identify gaps: missing inputs, unclear constraints, ambiguous behavior, codebase mismatches, contract concerns, or environment constraints.
4. Ask targeted questions when assumptions would materially affect behavior, architecture, data safety, or user experience.
5. If low-risk assumptions are enough to proceed, state them explicitly.
6. Produce a compact requirements list covering:
   - current behavior
   - desired behavior
   - invariants / must-not-break behavior
   - non-functional concerns
   - non-goals / scope boundaries
7. Confirm assumptions before substantial implementation.
