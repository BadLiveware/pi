...
- an explicit attempt cycle: check/measure current state -> change -> evaluate -> accept/reject/defer/split -> record -> repeat
...
- `.pi/loops/<loop-name>/attempt-archive.ndjson` for detailed/raw attempt data
- domain-specific negative-result docs for durable rejected/deferred lessons

For lightweight loops, the canonical file may be the only durable artifact; keep just enough anti-repeat memory to avoid retracing failed paths.
...
8. **Artifact Policy**: which outcomes need durable domain artifacts, which only need compact anti-repeat notes, and where each belongs.
9. **Compaction Policy**: when/how stale detail is archived while the canonical file stays small.
10. **Commit Policy**: how accepted kept state is committed or recorded when commit permission is active.
11. **Stop Rules**: only stop for explicit user stop, blockers, unsafe actions requiring approval, or agreed stop criteria.
...
4. Design the canonical loop file so a fresh agent can resume by reading one compact file.
5. Seed only a short rolling idea queue (1-3 active, a few deferred), keeping scope unbounded.
6. Define compaction rules so stale detail is archived and active context stays small.
7. Ensure the plan is loop-shaped, not phase-shaped; do not claim completion after backlog exhaustion.
8. Recommend `execute-plan` for execution; it will read its unbounded execution reference.
...
- Plan each Ralph iteration around one complete evaluated attempt by default.
- Allow up to 3 micro-attempts in one iteration only when each can be independently evaluated, logged, and decided.
