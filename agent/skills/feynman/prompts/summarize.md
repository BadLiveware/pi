...
- **Remote URL**: fetch to disk with `curl -sL -o .pi/feynman/notes/<slug>-raw.txt <url>`. Do NOT use fetch_content — its return value enters context directly, bypassing the RLM external-variable principle.
- **Local file or PDF**: copy or extract to `.pi/feynman/notes/<slug>-raw.txt`. For PDFs, extract text via `pdftotext` or equivalent before measuring.
...
- **Existing output**: if `.pi/feynman/outputs/<slug>-summary.md` already exists, ask the user whether to overwrite or use a different slug. Do not proceed until confirmed.
...
| > `<tier2-threshold>` | 3 | Full RLM — bash chunking + parallel `feynman-researcher` subagents |
...
Read `.pi/feynman/notes/<slug>-raw.txt` in full. Summarize directly using the output format. Write to `.pi/feynman/outputs/<slug>-summary.md`.
...
with open(".pi/feynman/notes/<slug>-raw.txt", encoding="utf-8") as f:
...
2. Append to `.pi/feynman/notes/<slug>-notes.md` before reading the next window. This is the checkpoint: if the session is interrupted, processed windows survive.
3. Log: `[summarize] window <N>/<total> done`

Synthesize `.pi/feynman/notes/<slug>-notes.md` into `.pi/feynman/outputs/<slug>-summary.md`.

---

## Tier 3 — Full RLM parallel chunks

Each chunk gets a fresh `feynman-researcher` subagent context window — context rot is impossible because no subagent sees more than `<window-size>` chars.
...
```python
import os
os.makedirs(".pi/feynman/notes", exist_ok=True)
...
for n, chunk in enumerate(chunks):
    ...
    with open(f".pi/feynman/notes/<slug>-chunk-{n:03d}.txt", "w", encoding="utf-8") as f:
        f.write(chunk)
...
Briefly summarize: "Source is ~<chars> chars -> <N> chunks -> <N> `feynman-researcher` subagents. This may take several minutes." Then continue automatically. Do not ask for confirmation or wait for a proceed response unless the user explicitly requested review before launching.

### 3c. Dispatch `feynman-researcher` subagents

```json
{
  "tasks": [{
    "agent": "feynman-researcher",
    "task": "Read ONLY `.pi/feynman/notes/<slug>-chunk-NNN.txt`. Extract: (1) key claims, (2) methodology or technical approach, (3) cited evidence. Do NOT use web_search or fetch external URLs — this is single-source summarization. If a claim appears to start or end mid-sentence at the file boundary, mark it BOUNDARY PARTIAL. Write to `.pi/feynman/notes/<slug>-summary-chunk-NNN.md`.",
    "output": ".pi/feynman/notes/<slug>-summary-chunk-NNN.md"
  }],
  "concurrency": 4
}
```
...
After all subagents return, verify every expected `.pi/feynman/notes/<slug>-summary-chunk-NNN.md` exists. Note any missing chunk indices — they will appear in the Coverage gaps section of the output. Do not abort on partial coverage; a partial summary with gaps noted is more useful than no summary.
...
Write to `.pi/feynman/outputs/<slug>-summary.md`.

---

## Output format

All tiers produce the same artifact at `.pi/feynman/outputs/<slug>-summary.md`:
...
Before you stop, verify on disk that `.pi/feynman/outputs/<slug>-summary.md` exists.

Sources contains only the single source confirmed reachable in Step 1. No `feynman-verifier` subagent is needed — there are no URLs constructed from memory to verify.
