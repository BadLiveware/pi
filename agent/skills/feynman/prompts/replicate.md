...
1. **Extract** — Use the `feynman-researcher` subagent to pull implementation details from the target paper and any linked code. If the workspace already uses `CHANGELOG.md` as an explicit research lab notebook, read the most recent relevant entries before planning or resuming.
...
5. **Log** — For multi-step or resumable replication work, append concise entries to `.pi/feynman/notes/<slug>-log.md` after meaningful progress, failed attempts, major verification outcomes, and before stopping. Record the active objective, what changed, what was checked, and the next step.
