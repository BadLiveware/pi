...
- Write durable or shared text for its future reader, not from the agent's chat provenance. For docs, README text, examples, comments, changelogs, commit messages, PR descriptions, issue replies, release notes, and generated reports, assume the reader has the repository or artifact but not the conversation, task list, implementation plan, or review thread. Explain the actual problem, behavior, invariant, trade-off, and validation context; do not write `as requested`, `as discussed`, `the user asked`, `from our conversation`, `this plan`, `phase/step/checklist`, `feedback`, `review comments`, or `CI findings` unless that process is itself the artifact's domain.
