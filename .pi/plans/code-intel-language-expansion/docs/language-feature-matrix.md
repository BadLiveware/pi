...
| Python | Python-specific outline/extractor, syntax impact/local/test routing, optional Pyrefly exact refs, and touched-file diagnostics are implemented with Pyrefly preferred, ty fallback, then basedpyright/pyright fallback. | Improve package/import resolution, nested/decorated edge cases, framework-aware test ranking, and Pyrefly workspace/config edge cases. | Pyrefly is the default Python exact-reference provider. `ty server`, pyright language server, or Jedi remain non-default alternatives only if Pyrefly is rejected by future fixture evidence. Ruff can be an optional lint diagnostics provider. |
